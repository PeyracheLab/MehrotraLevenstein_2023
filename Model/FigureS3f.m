function LabNotebook20211118a_WidthGradhSpeed(savepath)

%%
%savepath = '/Users/dl2820/Project Repos/UPDOWNGrad/LabNotebook/Figures_20211118a';
...
%I_in = -2.4;
gradmag = -logspace(-1,0,8);

width_line = logspace(-2,-0.5,8);
...
parfor nn = 1:numsims
    ...
    parms_temp{nn}.W = makeLineWeights(N_neurons,thiswidth,basicparms.W);
    parms_temp{nn}.beta = makeSlopeGradient(N_neurons,thisgrad,basicparms.beta);
...
for nn = 1:numsims
    [ff,gg,ww] = ind2sub([length(noisefrac),length(gradmag),length(width)],nn);
            if length(UDints_mean(ww,gg,ff).downints)<5
